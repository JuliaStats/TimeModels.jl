...
facts("Kalman Filter") do
    ...
    context("Time invariant models") do
        ...
        context("Missing data") do
            ...
            filt = filter(y, mod1)
            smoothed = smooth(y, mod1)
            @fact any(isnan(filt.x)) --> false
            @fact any(isnan(smoothed.x)) --> false
        end
        ...
        context("Return sizes") do
            ...
            @fact size(filt.x) --> size(filt.x_pred)
            @fact size(filt.x) --> size(smoothed.x)
            @fact size(filt.V) --> size(filt.V_pred)
            @fact size(filt.V) --> size(smoothed.V)
            @fact size(filt.V) --> size(smoothed.V_lag1)
        end
    end
    ...
    context("Time varying models") do
        ...
        context("Smoothing") do
            ...
            smoothed = smooth(y, sinusoid_model(4, fs = 8192, x0=[1.7, -0.2]) )
            @fact mean(smoothed.x, 1) --> roughly([0.5 -0.5]; atol= 0.1)

            #= x_est = round(smoothed.x[end, :], 3) =#
            #= display(lineplot(collect(1:size(x, 1)) / fs, vec(smoothed.x[1:end, 1]), width = 120, title="Smoothed State 1: $(x_est[1])")) =#
            #= display(lineplot(collect(1:size(x, 1)) / fs, vec(smoothed.x[1:end, 2]), width = 120, title="Smoothed State 2: $(x_est[2])")) =#
        end
    ...
    context("Linear regression test") do
        ...
        lm_filt = filter(y_noisy, lm, u=input)
        @fact lm_filt.x[end,1] --> roughly(y_true[end, 1], atol=4*sqrt(lm_filt.V[1,1,end]))
        lm_smooth = smooth(y_noisy, lm, u=input)
        @fact all((y_true - lm_smooth.x[:,1]) .< 4*sqrt(lm_smooth.V[1,1,:][:])) --> true
        @fact lm_smooth.x[1,2] --> roughly(lm_smooth.x[end, 2], atol=1e-12)
    end
